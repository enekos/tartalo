...
func (g *Generator) emitAgentRuntime() {
	...
	if g.usesLLM {
		g.writeLines([]string{
			...
			`# "gemini" calls Google's generateContent endpoint (requires curl`,
			`# + GEMINI_API_KEY); anything else pipes the prompt to`,
			`# $TARTALO_LLM_CMD (default: claude -p).`,
		...
		if g.testMode {
			...
			g.writeLines([]string{
				...
				`    gemini) __tt_llm_gemini "$__tt_p"; return $? ;;`,
				`  esac`,
				...
				`}`,
				"",
				`# __tt_llm_gemini calls Google's generateContent endpoint via curl.`,
				`# Defaults: base https://generativelanguage.googleapis.com/v1beta,`,
				`# model gemini-2.5-flash. Both are overridable via`,
				`# TARTALO_GEMINI_BASE_URL and TARTALO_GEMINI_MODEL — the URL override`,
				`# is also what makes tests point at a local server. curl is required;`,
				`# GEMINI_API_KEY is mandatory and is passed via the X-goog-api-key`,
				`# header (not ?key=) so the secret never lands in URL access logs.`,
				`# The two awk passes JSON-encode the outgoing prompt and decode the`,
				`# candidate's first .content.parts[0].text out of the response without`,
				`# pulling in jq or python.`,
				`__tt_llm_gemini() {`,
				`  __tt_p="$1"`,
				`  if [ -z "${GEMINI_API_KEY:-}" ]; then`,
				`    printf 'tartalo: gemini: GEMINI_API_KEY not set\n' >&2; exit 1`,
				`  fi`,
				`  if ! command -v curl >/dev/null 2>&1; then`,
				`    printf 'tartalo: gemini: curl is required on the shell target (use --target=native for a no-deps build)\n' >&2; exit 1`,
				`  fi`,
				`  __tt_base="${TARTALO_GEMINI_BASE_URL:-https://generativelanguage.googleapis.com/v1beta}"`,
				`  __tt_base="${__tt_base%/}"`,
				`  __tt_model="${TARTALO_GEMINI_MODEL:-gemini-2.5-flash}"`,
				`  __tt_enc=$(printf '%s' "$__tt_p" | awk '`,
				`    { if (NR > 1) out = out "\\n"`,
				`      n = length($0)`,
				`      for (i = 1; i <= n; i++) {`,
				...
				`    END { printf "%s", out }`,
				`  ')`,
				`  __tt_body=$(printf '{"contents":[{"role":"user","parts":[{"text":"%s"}]}]}' "$__tt_enc")`,
				`  __tt_resp=$(printf '%s' "$__tt_body" | curl -sS -X POST \`,
				`    -H "X-goog-api-key: ${GEMINI_API_KEY}" \`,
				`    -H "Content-Type: application/json" \`,
				`    --data-binary @- \`,
				`    "${__tt_base}/models/${__tt_model}:generateContent")`,
				`  __tt_curl_status=$?`,
				`  if [ "$__tt_curl_status" -ne 0 ]; then`,
				`    printf 'tartalo: gemini: curl failed (exit %s)\n' "$__tt_curl_status" >&2; exit 1`,
				`  fi`,
				`  __ret=$(printf '%s' "$__tt_resp" | awk '`,
				`    { buf = buf $0 }`,
				`    END {`,
				`      i = index(buf, "\"candidates\"")`,
				`      if (i == 0) { print "__TT_GEMINI_NO_CANDIDATES__"; exit }`,
				`      s = substr(buf, i)`,
				`      i = index(s, "\"parts\"")`,
				`      if (i == 0) { print "__TT_GEMINI_NO_CANDIDATES__"; exit }`,
				`      s = substr(s, i)`,
				`      i = index(s, "\"text\"")`,
				`      if (i == 0) { print "__TT_GEMINI_NO_CANDIDATES__"; exit }`,
				`      s = substr(s, i + length("\"text\""))`,
				`      while (length(s) > 0 && (substr(s,1,1) == " " || substr(s,1,1) == "\t" || substr(s,1,1) == "\n" || substr(s,1,1) == "\r" || substr(s,1,1) == ":")) s = substr(s, 2)`,
				`      if (substr(s, 1, 1) != "\"") { print "__TT_GEMINI_NO_CANDIDATES__"; exit }`,
				`      s = substr(s, 2)`,
				`      out = ""; esc = 0; n = length(s)`,
				`      for (i = 1; i <= n; i++) {`,
				`        c = substr(s, i, 1)`,
				...
				`    }`,
				`  ')`,
				`  if [ "$__ret" = "__TT_GEMINI_NO_CANDIDATES__" ]; then`,
				`    printf 'tartalo: gemini: no candidates in response: %s\n' "$__tt_resp" >&2; exit 1`,
				`  fi`,
				`}`,
				"",
			})
